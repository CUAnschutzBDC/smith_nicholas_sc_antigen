...
for(cells_use in c("all", "memory")){
  ...
  if(cells_use == "all"){
    vdj_dir <- file.path(save_dir, "images", "heavy_light_plots")
    vdj_files <- file.path(save_dir, "files", "heavy_light_files")
    ...
    vdj_dir <- file.path(save_dir, "images", "memory_heavy_light_plots")
    vdj_files <- file.path(save_dir, "files", "memory_heavy_light_files")
    seurat_data <- subset(seurat_data, 
                          subset = RNA_combined_celltype %in% 
                            c("Activated_memory", "Memory_IgA",
                              "Resting_memory"))
