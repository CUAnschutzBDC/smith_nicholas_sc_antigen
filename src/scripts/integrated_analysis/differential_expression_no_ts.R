...
# Remove TS
seurat_data <- subset(seurat_data, subset = sample != "JH310-12_TS")
...
ifelse(!dir.exists(file.path(save_dir, "images", "no_ts_pseudobulk")),
       dir.create(file.path(save_dir, "images", "no_ts_pseudobulk")),
       FALSE)
pdf(file.path(save_dir, "images", "no_ts_pseudobulk", "all_b_cells_capture_correct.pdf"))
...
openxlsx::saveWorkbook(wb = de_wb,
                       file = file.path(save_dir, "files",
                                        "no_ts_pseudobulk_all_bcell_de_capture_correct.xlsx"),
                       overwrite = TRUE)

saveRDS(pb, file.path(save_dir, "rda_obj", "no_ts_muscat_object_capture_correct.rds"))
...
all_res <- lapply(names(de_genes_all$table[[1]]), function(celltype){
  ...
  write.csv(all_tests, file.path(save_dir, "files",
                                 paste0("no_ts_pseudobulk_", celltype, "_de_capture_correct.csv")))
...
# Save the results
saveRDS(pb, file.path(save_dir, "rda_obj", "no_ts_mem_naive_muscat_object_capture_correct.rds"))
